class ObjFile
  attr_accessor :ignored_lines, :vertices, :default_group, :current_group, :groups
  ...
  PARSERS = [
    ...
    { pattern: /^f\s+(.*)$/, f: lambda { |data, match|
                                  vertex_indices = match[1].split.map(&:to_i)
                                  shapes = data.create_shape(vertex_indices.map { |i| data.vertices[i] })
  ...
  def parse(content)
    ...
    result = lines.reduce(self) do |acc, line|
  ...
  def create_shape(vertices)
    return [Triangle.new(vertices)] if vertices.length == 3
  ...
  def fan_triangulation(vertices)
    (1..(vertices.length - 2)).map do |i|
      Triangle.new([vertices[0], vertices[i], vertices[i + 1]])
    end
  end
end
